...
static Naui_TextRenderer g_text_renderer;
static Naui_ButtonRenderer g_button_renderer;
static Naui_ToggleRenderer g_toggle_renderer;
static Naui_DropDownRenderer g_dropdown_renderer;
static Naui_SliderIntRenderer g_slider_int_renderer;
static Naui_SliderFloatRenderer g_slider_float_renderer;
static Naui_DragIntRenderer g_drag_int_renderer;
static Naui_DragFloatRenderer g_drag_float_renderer;
static Naui_TextFieldRenderer g_text_field_renderer;
static Naui_KnobRenderer g_knob_renderer;
...
void naui_widget_init(void)
{
	g_text_renderer = naui_widget_text_render;
	g_button_renderer = naui_widget_button_render;
	g_toggle_renderer = naui_widget_toggle_render;
	g_dropdown_renderer = naui_widget_dropdown_render;
	g_slider_int_renderer = naui_widget_slider_int_render;
	g_slider_float_renderer = naui_widget_slider_float_render;
	g_drag_int_renderer = naui_widget_drag_int_render;
	g_drag_float_renderer = naui_widget_drag_float_render;
	g_text_field_renderer = naui_widget_text_field_render;
	g_knob_renderer = naui_widget_knob_render;
}
...
void naui_widget_render_set(Naui_WidgetType widget_type, void *renderer)
{
	switch (widget_type)
	{
		case NAUI_WIDGET_TEXT:
			g_text_renderer = renderer ? (Naui_TextRenderer)renderer : naui_widget_text_render;
			break;

		case NAUI_WIDGET_BUTTON:
			g_button_renderer = renderer ? (Naui_ButtonRenderer)renderer : naui_widget_button_render;
			break;

		case NAUI_WIDGET_TOGGLE:
			g_toggle_renderer = renderer ? (Naui_ToggleRenderer)renderer : naui_widget_toggle_render;
			break;

		case NAUI_WIDGET_DROPDOWN:
			g_dropdown_renderer = renderer ? (Naui_DropDownRenderer)renderer : naui_widget_dropdown_render;
			break;

		case NAUI_WIDGET_SLIDER_INT:
			g_slider_int_renderer = renderer ? (Naui_SliderIntRenderer)renderer : naui_widget_slider_int_render;
			break;

		case NAUI_WIDGET_SLIDER_FLOAT:
			g_slider_float_renderer = renderer ? (Naui_SliderFloatRenderer)renderer : naui_widget_slider_float_render;
			break;

		case NAUI_WIDGET_DRAG_INT:
			g_drag_int_renderer = renderer ? (Naui_DragIntRenderer)renderer : naui_widget_drag_int_render;
			break;

		case NAUI_WIDGET_DRAG_FLOAT:
			g_drag_float_renderer = renderer ? (Naui_DragFloatRenderer)renderer : naui_widget_drag_float_render;
			break;

		case NAUI_WIDGET_TEXT_FIELD:
			g_text_field_renderer = renderer ? (Naui_TextFieldRenderer)renderer : naui_widget_text_field_render;
			break;

		case NAUI_WIDGET_KNOB:
			g_knob_renderer = renderer ? (Naui_KnobRenderer)renderer : naui_widget_knob_render;
			break;
	}
}
...
bool naui_toggle(const char *label, bool *value)
{
	...
	g_toggle_renderer(id, label, value);
	return clicked;
}
...
bool naui_dropdown(const char* label, uint32_t* select, const char** items, size_t item_count)
{
	...
	g_dropdown_renderer(id, label, select, items, item_count);
...
bool naui_slider_int(const char *label, int32_t *value, int32_t min, int32_t max)
{
	...
	g_slider_int_renderer(id, label, value, min, max);
...
bool naui_slider_float(const char *label, float *value, float min, float max, const char *fmt)
{
	...
	g_slider_float_renderer(id, label, value, min, max, fmt);
	return original != *value;
}
...
bool naui_drag_int(const char *label, int *value, int speed, int min, int max)
{
	...
	g_drag_int_renderer(id, label, value, speed, min, max);
	return original != *value;
}
...
bool naui_drag_float(const char *label, float *value, float speed, float min, float max, const char *fmt)
{
	...
	g_drag_float_renderer(id, label, value, speed, min, max, fmt);
	return original != *value;
}
...
bool naui_text_field(const char *label, const char* hint, char *buffer, size_t buffer_size)
{
	...
	g_text_field_renderer(id, label, hint, buffer, buffer_size);
...
bool naui_knob(const char *label, float *value, float min, float max, const char *fmt)
{
	...
	g_knob_renderer(id, label, value, min, max, fmt);
	return original != *value;
}