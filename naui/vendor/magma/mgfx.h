...
#define MGFX_D3D11_SAFE_RELEASE(obj) if (obj) { (obj)->lpVtbl->Release(obj); obj = NULL; }
    ...
    static void mgfx_d3d11_init(const mgfx_init_info *init_info)
    {
        ...
        DXGI_SWAP_CHAIN_DESC sd = {
            .BufferCount = 1,
            .BufferDesc.Width = init_info->width,
            .BufferDesc.Height = init_info->height,
            .BufferDesc.Format = DXGI_FORMAT_B8G8R8A8_UNORM,
            .BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT,
            .OutputWindow = (HWND)init_info->handle->win32.hwnd,
            .SampleDesc.Count = 1,
    ...
    static void mgfx_d3d11_resize_backbuffer(void)
    {
        ...
        HRESULT hr = IDXGISwapChain_ResizeBuffers(ctx.d3d11.swapchain, 0, ctx.d3d11.width, ctx.d3d11.height, DXGI_FORMAT_B8G8R8A8_UNORM, 0);
        ...
        hr = IDXGISwapChain_GetBuffer(ctx.d3d11.swapchain, 0, &IID_ID3D11Texture2D, (void**)&backbuffer);
        
        if (FAILED(hr))
            return;
        
        hr = ID3D11Device_CreateRenderTargetView(ctx.d3d11.device, (ID3D11Resource*)backbuffer, NULL, &ctx.d3d11.target_view);
    ...
    static void mgfx_d3d11_update_buffer(mgfx_d3d11_buffer *buffer, size_t offset, size_t size, void *data)
    {
        ...
        ID3D11DeviceContext_UpdateSubresource(ctx.d3d11.immediate_context, (ID3D11Resource*)buffer->buffer, 0, &box, data, 0, 0);
    }
    ...
    static void mgfx_d3d11_update_image(mgfx_d3d11_image *image, size_t size, void *data)
    {
        if (image->is_cpu)
        {
            ...
            for (uint32_t y = 0; y < image->height; y++)
                memcpy((uint8_t*)mapped.pData + y * mapped.RowPitch,
                       ...
                       row_pitch);
            ID3D11DeviceContext_Unmap(ctx.d3d11.immediate_context, (ID3D11Resource*)image->texture, 0);
            return;
        }
        ...
        ID3D11DeviceContext_UpdateSubresource(ctx.d3d11.immediate_context, (ID3D11Resource*)image->texture, 0, NULL, data, row_pitch, depth_pitch);
    }
    ...
    static void mgfx_d3d11_bind_pass(const mgfx_pass_info *pass)
    {
        ...
        ID3D11DeviceContext_OMSetRenderTargets(ctx.d3d11.immediate_context, color ? 1 : 0, color ? &color->rtv : NULL, depth ? depth->dsv : NULL);
        ...
        if (depth)
            ID3D11DeviceContext_ClearDepthStencilView(ctx.d3d11.immediate_context, depth->dsv, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
    }
    ...
    static mgfx_d3d11_pipeline *mgfx_d3d11_create_pipeline(const mgfx_pipeline_create_info *create_info)
    {
        ...
        if (create_info->shader.compute.size)
        {
            ...
            ID3D11Device_CreateComputeShader(ctx.d3d11.device, ID3D10Blob_GetBufferPointer(cs_blob), ID3D10Blob_GetBufferSize(cs_blob), NULL, &pipeline->compute_shader);
        ...
        if (create_info->vertex_attributes[0])
        {
            ...
            ID3D11Device_CreateInputLayout(ctx.d3d11.device, layout, attribute_count, vs_buffer_ptr, vs_buffer_size, &pipeline->vertex_layout);
        ...
        ID3D11Device_CreateRasterizerState(ctx.d3d11.device, &raster_desc, &pipeline->raster_state);
        ...
        ID3D11Device_CreateDepthStencilState(ctx.d3d11.device, &depth_stencil_state_desc, &pipeline->depth_stencil_state);
